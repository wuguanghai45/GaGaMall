...
class StoreDetails extends Component {
  ...
  onPressItemRight(){
    const { navigation } = this.props;
    InteractionManager.runAfterInteractions(() => {
      navigation.navigate('GoodDetails');
    });
  }
     ...
     return (
       <TouchableOpacity onPress={()=>{this.topItemAction()}}>
         <View style={{height: PARALLAX_HEADER_HEIGHT, flex: 1, alignItems: 'center', justifyContent: 'center' }}>
              <ImageBackground source={require('../imgs/store/ic_store_top_bg.png')} style={{width:width,height:PARALLAX_HEADER_HEIGHT}}>
                  <View style={{flexDirection:'row',marginLeft:24,height:68,alignItems:'center',marginTop:12}}>
                       ...
                       <Text style={{color:'white',fontSize:16,marginLeft:26}}>{data.name}</Text>
